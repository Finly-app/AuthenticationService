...
public class UserUpdatedConsumer : BackgroundService {
    ...
    private readonly IUserUpdatedHandler _handler;

    public UserUpdatedConsumer(
        ILogger<UserUpdatedConsumer> logger,
        IConfiguration config,
        IUserUpdatedHandler handler) {
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken) {
        _consumer.Subscribe("user-updated-topic");
        ...
        return Task.Run(async () => {
            while (!stoppingToken.IsCancellationRequested) {
                try {
                    ...
                    var userUpdated = JsonConvert.DeserializeObject<UserUpdatedEvent>(result.Message.Value);
                    await _handler.HandleAsync(userUpdated, hmacSecret, _producer);
                } catch (Exception ex) {
